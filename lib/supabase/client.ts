...
export function getSupabaseClient(){
  ...
  const url=validSupabaseUrl(configuredUrl)?configuredUrl!.trim():deployedSupabaseUrl;
  const key=validPublishableKey(configuredKey)?configuredKey!.trim():deployedPublishableKey;
